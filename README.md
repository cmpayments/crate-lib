...
[![Build Status][]](https://travis-ci.org/cmpayments/crate-lib)

Crate is a library that is forked from the [Box project](https://github.com/box-project/box2-lib) and it is
built on the [`Phar`][] class. It is designed to make it
...
The Crate library includes many features and some are designed so that they can
...
Crate makes uses of source file "compactors". A compactor is simply a class
...
However, the `phar` extension is required to do either task. The Crate library
...
If appropriate for the project, a custom stub can be generated by the Crate
...
the `Crate` class. The `Crate` class is designed to automatically integrate all
...
> Note that the `Crate::create()` method accepts the same arguments as the
> `Phar::__construct()` method.

#### Registering Compactors

Whether you are using the bundled compactors or your own, you will need to
call the `Crate->addCompactor()` method to register your class with `Crate`.
All files added to the phar using `Crate` will be automatically run through
...
The `Crate` class offers the ability to search and replace placeholder values
...
replacements, you will need to use the `Crate` class's methods for adding files.
...
The `Crate` class offers a simple way of adding a stub sourced from a file, and
...
The `Crate` class offers two simple ways of signing the phar using a private
...
[Build Status]: https://travis-ci.org/cmpayments/crate-lib.png?branch=master
[`Phar`]: http://us3.php.net/manual/en/class.phar.php
[`CompactorInterface`]: src/lib/CmPayments/Crate/Compactor/CompactorInterface.php
